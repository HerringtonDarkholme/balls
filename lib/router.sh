...
balls::route() {
  ...
  local action=$(echo "$ROUTES" | grep "^$REQUEST_METHOD" | {
    while read line; do
      arr=($line); path=${arr[1]}; act=${arr[2]}
      if [[ "$REQUEST_PATH" = "$path" ]]; then echo $act; break; fi
    done
  })
  ...
  if [ -n "$action" ] && exists "$action"; then
    ...
    ( $action 3>$headers_sock ) | {
      ...
      body=$(cat -)
      response=$(echo "$headers"; echo "Content-Length: ${#body}"; echo; echo "$body")
      echo "$response"
    }
    rm -f "$headers_sock"
  else
